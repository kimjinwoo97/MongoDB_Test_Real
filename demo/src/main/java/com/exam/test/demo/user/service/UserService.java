...
public class UserService {
  private final UserRepository userRepository; // final로 선언
  ...
  public ResponseDto<UserDto> getUserInfo(String id) throws CustomException {
    ...
    if (userOptional.isPresent()) {
      ...
      UserDto userDto = new UserDto(user.getUserId(), null, user.getUserName(), user.getRoles()); // 비밀번호늰 노출되지 않음
      return new ResponseDto<>(userDto, "상세 정보를 확인해 보세요.");
    } else {
      throw new CustomException("헤딩 사용자를 찾을 수 없습니다.");
    }
  }
